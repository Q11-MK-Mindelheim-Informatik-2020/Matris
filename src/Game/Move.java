...
public class Move {
    ...
    static void removeLines(ArrayList<Integer> lines) throws InterruptedException {
        ...
        if (l == 4) {
            Sounds.playSound("NES - tetris clear.mp3", 0.7, false);
        }
        else {
            Sounds.playSound("NES - line clear.mp3", 0.7, false);
        }
    ...
    public static boolean store() {
        if (Var.gameState.equals("singleplayer") && !Var.stored) {
            Mechanics.resetTimer();
            Effects.Sounds.playSound("NES - store.mp3", 0.7, false);
            ...
            //Effects.Sounds.playSound("NES - store lock.mp3", 0.7, false);
            return false;
        }
    }
}